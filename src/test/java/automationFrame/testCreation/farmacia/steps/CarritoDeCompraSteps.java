package automationFrame.testCreation.farmacia.steps;
...
public class CarritoDeCompraSteps {
    ...
    CarritoDeCompraPage carritoDeCompraPages = new CarritoDeCompraPage(DriverFactory.getDriver());

    @Given("que estoy en la url {string}")
    public void que_estoy_en_la_url( String url) {
        carritoDeCompraPages.navigateToFarmacia(url);
    }

    @When("presiono el boton guardar")
    public void presiono_el_boton_guardar() {
        carritoDeCompraPages.clickInBtnProduct();
    }
